...
@section('content')
    <div class="space-y-6 max-w-full">
        ...
        <div class="flex items-center gap-4">
            <flux:button href="{{ route('clients.index') }}" variant="ghost" class="!p-2">
                <flux:icon icon="arrow-left" class="w-5 h-5" />
            </flux:button>
            <div>
                <flux:heading size="xl">Tambah Klien</flux:heading>
                <flux:subheading>Tambah klien atau partner baru</flux:subheading>
            </div>
        </div>
        ...
        <flux:card>
            <form action="{{ route('clients.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <flux:field>
                    ...
                    <flux:input name="name" value="{{ old('name') }}" placeholder="Masukkan nama klien" required />
                    <flux:error name="name" />
                </flux:field>

                <!-- Photo -->
                <flux:field>
                    <flux:label>Logo Klien</flux:label>
                ...
                <flux:field>
                    ...
                    <flux:description>Pilih ukuran tampilan logo klien di halaman depan</flux:description>
                </flux:field>

                <!-- Actions -->
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                    <flux:button href="{{ route('clients.index') }}" variant="ghost">Batal</flux:button>

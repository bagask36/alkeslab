...
@section('content')
    <div class="space-y-8 max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-4 pb-2 border-b border-zinc-200 dark:border-zinc-700">
            ...
            <div class="flex-1">
                <flux:heading size="xl" class="!mb-1">Edit Klien</flux:heading>
                <flux:subheading class="!mt-0">Edit data klien atau partner</flux:subheading>
            </div>
        </div>
            ...
            <form action="{{ route('clients.update', $client->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <flux:field>
                        ...
                        <flux:input name="name" value="{{ old('name', $client->name) }}" placeholder="Masukkan nama klien" required class="!rounded-xl !border-zinc-300 dark:!border-zinc-600" />
                ...
                <div class="space-y-6">
                    ...
                    <flux:field>
                        ...
                        @if($client->image)
                            <div class="mb-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                                <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Logo saat ini:</p>
                                <img src="{{ asset('storage/' . $client->image) }}" alt="Current logo" class="max-w-xs rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                            </div>
                        @endif
                        ...
                        <div id="photo-preview" class="mt-4 hidden">
                            <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Preview logo baru:</p>
                        ...
                        <flux:description class="!mt-2">Kosongkan jika tidak ingin mengubah logo</flux:description>
                    </flux:field>
                    ...
                    <flux:field>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-2">
                            <label class="relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:border-[var(--color-accent)] {{ old('image_size', $client->image_size) == 'small' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/5' : 'border-zinc-300 dark:border-zinc-600' }}">
                                <input type="radio" name="image_size" value="small" class="sr-only" {{ old('image_size', $client->image_size) == 'small' ? 'checked' : '' }} />
                            ...
                            <label class="relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:border-[var(--color-accent)] {{ old('image_size', $client->image_size) == 'large' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/5' : 'border-zinc-300 dark:border-zinc-600' }}">
                                <input type="radio" name="image_size" value="large" class="sr-only" {{ old('image_size', $client->image_size) == 'large' ? 'checked' : '' }} />
                                <div class="flex-1 text-center">

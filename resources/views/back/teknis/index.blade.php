...
@section('content')
    <div class="space-y-6 max-w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <flux:heading size="xl">Teknis</flux:heading>
                <flux:subheading>Kelola data teknis dan spesialisasi</flux:subheading>
            </div>
            <flux:button href="{{ route('teknis.create') }}">
        ...
        <!-- Statistics -->
        @php
            $totalTeknis = \App\Models\Teknis::count();
            $smallTeknis = \App\Models\Teknis::where('image_size', 'small')->count();
            $largeTeknis = \App\Models\Teknis::where('image_size', 'large')->count();
        @endphp
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-zinc-600 dark:text-zinc-400">Total Teknis</p>
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $totalTeknis }}</p>
                    </div>
                    <div class="p-3 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                        <flux:icon icon="cog-6-tooth" class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                    </div>
                </div>
            </flux:card>
            ...
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $smallTeknis }}</p>
                    </div>
            ...
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $largeTeknis }}</p>
                    </div>
        ...
        <flux:card>
            <div class="overflow-x-auto">
                <table id="teknis-table" class="w-full">
                    <thead>
    ...
    <script>
        window.addEventListener('load', function() {
            ...
            jQuery(document).ready(function($) {
                ...
                if ($('#teknis-table').length === 0) {
                    console.error('Table #teknis-table not found');
                    return;
                }

                console.log('Initializing Teknis DataTable...');
                console.log('AJAX URL:', "{{ route('teknis.index') }}");
                ...
                columns: [
                    ...
                    initComplete: function() {
                        console.log('Teknis DataTable initialized successfully');
                    }
                }).on('xhr.dt', function (e, settings, json, xhr) {
                    console.log('Teknis DataTables AJAX request completed:', xhr.status);
                });
    ...
    <style>
        #teknis-table_wrapper .dataTables_filter input,
        #teknis-table_wrapper .dataTables_length select {
        ...
        #teknis-table_wrapper .dataTables_paginate .paginate_button {

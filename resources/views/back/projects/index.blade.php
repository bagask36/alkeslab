...
@section('content')
    <div class="space-y-6 max-w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <flux:heading size="xl">Proyek</flux:heading>
                <flux:subheading>Kelola portfolio dan proyek perusahaan</flux:subheading>
            </div>
        ...
        @php
            ...
            $smallProjects = \App\Models\Project::where('image_size', 'small')->count();
            $largeProjects = \App\Models\Project::where('image_size', 'large')->count();
        @endphp
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-zinc-600 dark:text-zinc-400">Total Proyek</p>
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $totalProjects }}</p>
                    </div>
                    <div class="p-3 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                        <flux:icon icon="folder" class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                    </div>
                </div>
            </flux:card>
            ...
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $smallProjects }}</p>
                    </div>
            ...
            <flux:card>
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ $largeProjects }}</p>
                    </div>
        ...
        <flux:card>
            <div class="overflow-x-auto">
                <table id="projects-table" class="w-full">
                    <thead>
        ...
        window.addEventListener('load', function() {
            ...
            jQuery(document).ready(function($) {
                ...
                if ($('#projects-table').length === 0) {
                    console.error('Table #projects-table not found');
                    return;
                }

                console.log('Initializing Projects DataTable...');
                console.log('AJAX URL:', "{{ route('projects.index') }}");
                ...
                columns: [
                    ...
                    console.log('Projects DataTables AJAX request completed:', xhr.status);
                });
    ...
    <style>
        #projects-table_wrapper .dataTables_filter input,
        #projects-table_wrapper .dataTables_length select {
        ...
        #projects-table_wrapper .dataTables_paginate .paginate_button {
        ...
        #projects-table_wrapper .dataTables_paginate .paginate_button.current {
            @apply bg-[var(--color-accent)] text-white hover:bg-[color-mix(in_oklab,_var(--color-accent),_transparent_10%)];
        }
        #projects-table_wrapper .dataTables_info {

...
@section('content')
    <div class="space-y-8 max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-4 pb-2 border-b border-zinc-200 dark:border-zinc-700">
            <flux:button href="{{ route('projects.index') }}" variant="ghost" class="!p-2 hover:!bg-zinc-100 dark:hover:!bg-zinc-800">
                <flux:icon icon="arrow-left" class="w-5 h-5" />
            </flux:button>
            <div class="flex-1">
                <flux:heading size="xl" class="!mb-1">Edit Proyek</flux:heading>
                <flux:subheading class="!mt-0">Edit proyek di portfolio</flux:subheading>
            </div>
        </div>
            ...
            <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-700">
                        ...
                        <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Informasi Proyek</h3>
                    </div>

                    <!-- Name -->
                    <flux:field>
                        <flux:label class="!text-sm !font-semibold">Nama Proyek</flux:label>
                        <flux:input name="name" value="{{ old('name', $project->name) }}" placeholder="Masukkan nama proyek" required class="!rounded-xl !border-zinc-300 dark:!border-zinc-600" />
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-2 pb-2 border-b border-zinc-200 dark:border-zinc-700">
                        ...
                        <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Gambar Proyek</h3>
                    </div>
                    ...
                    <flux:field>
                        ...
                        @if($project->image)
                            <div class="mb-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                                ...
                                <img src="{{ asset('storage/' . $project->image) }}" alt="Current image" class="max-w-xs rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                            </div>
                        @endif
                    ...
                    <flux:field>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-2">
                            <label class="relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:border-[var(--color-accent)] {{ old('image_size', $project->image_size) == 'small' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/5' : 'border-zinc-300 dark:border-zinc-600' }}">
                                <input type="radio" name="image_size" value="small" class="sr-only" {{ old('image_size', $project->image_size) == 'small' ? 'checked' : '' }} />
                            ...
                            <label class="relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:border-[var(--color-accent)] {{ old('image_size', $project->image_size) == 'large' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/5' : 'border-zinc-300 dark:border-zinc-600' }}">
                                <input type="radio" name="image_size" value="large" class="sr-only" {{ old('image_size', $project->image_size) == 'large' ? 'checked' : '' }} />
                                <div class="flex-1 text-center">
                        ...
                        <flux:description class="!mt-2">Pilih ukuran tampilan gambar proyek di halaman depan</flux:description>
                    </flux:field>
                </div>

                <!-- Actions -->
                <div class="flex items-center justify-between pt-6 border-t border-zinc-200 dark:border-zinc-700">
                    <flux:button href="{{ route('projects.index') }}" variant="ghost" class="!px-6">

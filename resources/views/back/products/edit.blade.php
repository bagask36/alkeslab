...
@section('content')
    <div class="space-y-8 max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-4 pb-2 border-b border-zinc-200 dark:border-zinc-700">
            ...
            <div class="flex-1">
                <flux:heading size="xl" class="!mb-1">Edit Produk</flux:heading>
                <flux:subheading class="!mt-0">Edit produk di katalog</flux:subheading>
            </div>
        </div>
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <flux:field>
                        ...
                        <flux:textarea name="description" rows="5" placeholder="Masukkan deskripsi produk" required class="!rounded-xl !border-zinc-300 dark:!border-zinc-600">{{ old('description', $product->description) }}</flux:textarea>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <flux:field>
                            ...
                            @if($product->image)
                                <div class="mb-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                                    ...
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="Current photo" class="max-w-full rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                                </div>
                            @endif
                            <label for="image" class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-zinc-300 dark:border-zinc-600 rounded-xl cursor-pointer bg-zinc-50 dark:bg-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-4 pb-4">
                                    <flux:icon icon="photo" class="w-10 h-10 mb-2 text-zinc-400 dark:text-zinc-500" />
                                    <p class="mb-1 text-xs text-zinc-500 dark:text-zinc-400"><span class="font-semibold">Upload foto baru</span></p>
                            ...
                            <div id="image-preview" class="mt-3 hidden">
                                ...
                                <img id="image-preview-img" src="" alt="Preview" class="max-w-full rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                            </div>
                        ...
                        <flux:field>
                            ...
                            @if($product->icon)
                                <div class="mb-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                                    <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Icon saat ini:</p>
                                    <img src="{{ asset('storage/' . $product->icon) }}" alt="Current icon" class="max-w-full rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                                </div>
                            @endif
                            <label for="icon" class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-zinc-300 dark:border-zinc-600 rounded-xl cursor-pointer bg-zinc-50 dark:bg-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-4 pb-4">
                                    ...
                                    <p class="mb-1 text-xs text-zinc-500 dark:text-zinc-400"><span class="font-semibold">Upload icon baru</span></p>
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">JPG, PNG, GIF (MAX. 2MB)</p>
                                </div>
                                <input id="icon" type="file" name="icon" accept="image/*" class="hidden" />
                            </label>
                            <div id="icon-preview" class="mt-3 hidden">
                                <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Preview icon baru:</p>
                                <img id="icon-preview-img" src="" alt="Preview" class="max-w-full rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                            </div>
                            <flux:error name="icon" />
                            <flux:description class="!mt-2">Kosongkan jika tidak ingin mengubah icon</flux:description>
                        </flux:field>
                    </div>
                </div>
